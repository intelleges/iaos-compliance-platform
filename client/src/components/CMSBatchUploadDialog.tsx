 ...
 * 
 * 3-step wizard for bulk multi-language content import:
...
import { Upload, FileSpreadsheet, AlertCircle, CheckCircle2, XCircle, Languages } from 'lucide-react';
import { toast } from 'sonner';

interface CMSBatchUploadDialogProps {
  open: boolean;
...
export function CMSBatchUploadDialog({
  open,
  onOpenChange,
  onSuccess,
}: CMSBatchUploadDialogProps) {
  ...
  const validateMutation = trpc.cmsBatch.validate.useMutation();
  const uploadMutation = trpc.cmsBatch.upload.useMutation();
  ...
  const handleValidate = async () => {
    ...
    try {
      const result = await validateMutation.mutateAsync({ fileBase64, languageCode });
  ...
  const handleImport = async () => {
    ...
    try {
      const result = await uploadMutation.mutateAsync({ fileBase64, languageCode });
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Import CMS Content</DialogTitle>
          <DialogDescription>
            Bulk import multi-language page content (INT.DOC.64 Section 6.3)
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <p className="text-lg font-medium mb-2">
                {file ? file.name : 'Drop your CMS Template here'}
              </p>
            ...
            <div className="p-4 bg-gray-50 rounded-lg border">
              <div className="mb-3 space-y-2">
                ...
                <div className="flex gap-4">
                  <a 
                    href="/templates/Questionnaire_CMS_Import_Template_Blank.xlsx" 
                    download
                  ...
                  <a 
                    href="/templates/Questionnaire_CMS_Import_Template_Sample.xlsx" 
                    download
              ...
              <p className="text-xs text-gray-600">
                Element Key, Page Name, Content (English), Content (Spanish), Is Active, Description
              </p>
            </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-blue-700">
                    {(file.size / 1024).toFixed(1)} KB • Language: {languageCode}
                  </p>
                </div>
                <Button
                  onClick={() => {
        ...
        {step === 'validate' && validationResult && (
          <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-700">
                      All {validationResult.summary.validRows} content entries are ready to import
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              >
                {uploadMutation.isPending ? 'Importing...' : 'Import Content'}
              </Button>
            </div>
          </div>
        )}
        ...
        {step === 'import' && importResult && (
          <div className="space-y-4">
            <div className="p-6 bg-green-50 rounded-lg border border-green-200 text-center">
              <Languages className="h-16 w-16 mx-auto mb-4 text-green-600" />
              <h3 className="text-xl font-bold text-green-900 mb-2">Import Complete!</h3>
              <p className="text-green-700">
                Successfully processed {importResult.summary.totalProcessed} content entries
              </p>
            </div>
              ...
              <div className="max-h-48 overflow-y-auto border rounded-lg">
                ...
                <div className="divide-y">
                  {importResult.errors.map((error: any, idx: number) => (
                    <div key={idx} className="px-4 py-3">
                      <p className="font-medium text-red-900">
                        Row {error.rowNumber}: Page {error.questionnaireCMS} - {error.description}
                      </p>
